...
const (
	...
	getQuery     = `SELECT * FROM accounts WHERE user_id = $1`
)
...
func (adb *AccountDB) Get(id uuid.UUID) (account *models.Account, err error) {
...
func (adb *AccountDB) AddWith(t Transaction, account *models.Account) (err error) {
	tx, ok := t.(*sqlx.Tx)
	if !ok {
		return SqlxError
	}
...
func (adb *AccountDB) Add(account *models.Account) (err error) {
	_, err = adb.db.NamedExec(addQuery, account)

	return
}
func (adb *AccountDB) All() (f []*models.Account, err error) {
...
func (adb *AccountDB) PayWith(t Transaction, payment *models.Payment) (err error) {
	tx, ok := t.(*sqlx.Tx)
	if !ok {
		return SqlxError
	}
